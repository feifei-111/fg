...
GLFWWindowImpl::GLFWWindowImpl(const WindowConfig& config, WindowBase* base){
    ...
    // --------------------
    window_ = glfwCreateWindow(config.width, config.height, config.name, NULL, NULL);
...
void GLFWWindowImpl::SwapBuffer() const {
...
namespace {
    ...
    void FramebufferSizeCallback(GLFWwindow* window, int width, int height) {
        auto window_impl = reinterpret_cast<GLFWWindowImpl*>(glfwGetWindowUserPointer(window));
    ...
    void KeyBoardCallback(GLFWwindow* window, int key, int scancode, int action, int mods){
        ...
        switch(action){
            ...
            case GLFW_RELEASE:
                ...
                break;
            default:
                CHECK(false) << "Unexcepted keyboard button move for glfw: " << action;
        }
