...
LRESULT CALLBACK WindowProc(HWND hwnd,
    ...
    Win32WindowImpl* window_impl = reinterpret_cast<Win32WindowImpl*>GetWindowLongPtrW(hwnd, GWLP_USERDATA);

    // 初始化未完成
    if (!window_impl->create_window_ready_flag_) {
        switch (uMsg) {
            case WM_NCCREATE:
                auto user_data = (LONG_PTR)(((CREATESTRUCT*)lParam)->lpCreateParams);
                CHECK(user_data)
