...
void UnregisterWindow(size_t window_id) { GlobalWindowRegMap.erase(window_id); }
...
struct EventList {
    ...
    bool PushEvent(Event* event) {
        VLOG(7) << "PushEvent: " << event->type << ", " << event->window_id
                << ", " << event->time_stamp;
        if (count_ == Capacity) {
            return false;
        }
...
bool PushEvent(Event* event) { return GlobalEventList.PushEvent(event); }
    ...
    switch (event->type) {
            ...
            switch (event->data.mouse_click_event.move) {
                case ButtonMove::DOWN:
                    state->button_map[size_t(
                      event->data.mouse_click_event.button)] = true;
                    break;
                case ButtonMove::UP:
                    state->button_map[size_t(
                      event->data.mouse_click_event.button)] = false;
                    break;
            }
            break;
        ...
        case EventType::kKeyboardEvent:
            switch (event->data.keyboard_event.move) {
                case ButtonMove::DOWN:
                    state
                      ->button_map[size_t(event->data.keyboard_event.button)] =
                      true;
                    break;
                case ButtonMove::UP:
                    state
...
bool PollEvent(Event* event) {
    ...
    if (GlobalEventList.PopHeadEvent(event)) {
        // std::cout << event->type  << ", " << event->window_id << ", "
        // <<event->time_stamp << std::endl;
        CHECK(GlobalWindowRegMap.find(event->window_id) !=
              GlobalWindowRegMap.end())
          << "window id " << event->window_id << " not registered !!!";
