...
Mesh::Mesh(aiMesh* ai_mesh) {
    ...
    if(VLOG_IS_ON(8)){
        std::ostringstream ss;
        unsigned int idx=0;
        VLOG(8) << "num vertex" << num_vertices_ << " data_width " << data_width << " vbo_buf.size() " << vbo_buf.size();
        VLOG(8) << "vbo buf";
...
Material::Material(aiMaterial* material, Scene* scene){
    for(unsigned int mat_type_val=1; mat_type_val<=11; mat_type_val++){
        ...
        for(unsigned int i = 0; i < material->GetTextureCount(mat_type); i++){
            ...
            if (scene->GetTextures()->find(texture_path) == scene->GetTextures()->end()){
...
SceneNode* SceneNode::FindNode(const std::string& node_name){
    if (name_ == node_name) return this;
    else{
...
static void TreeDebugStrImpl(const SceneNode* node, std::ostringstream* log, std::vector<int>* prefix){
...
void Scene::DrawMesh(unsigned int mesh_idx, const fg_gl::ShaderProgram& program) {
    ...
    VLOG(7) << "DrawMesh With: " 
        << mesh->GetNumVertices() << " vertices, " << mesh->GetNumFaces() << " faces \n"
        << materials_[mesh->GetMaterialIdx()].DebugStr();

    for(const auto& pair_data : materials_[mesh->GetMaterialIdx()].GetTexturePathes()){
        ...
        for (const std::string& path : pair_data.second){
            ...
            VLOG(6) << "Bind " << path << " as " << ss.str() << " to unit " << textures_[path].CurrentUnit();
...
static void DrawSceneNode(SceneNode* node, Scene* scene, bool recursively, const fg_gl::ShaderProgram& program){
...
void Scene::DrawNode(const std::string& node_name, const fg_gl::ShaderProgram& program, bool recursively){
    SceneNode* node = FindNode(node_name);
    CHECK(node) << "DrawNode " << node_name <<  " but " << node_name << " not exist!!";
    DrawSceneNode(node, this, recursively, program);
}

void Scene::DrawNode(SceneNode* node, const fg_gl::ShaderProgram& program, bool recursively){
    SceneNode* found_node = FindNode(node->GetName());
    CHECK(found_node == node) << "DrawNode with pointer, but found node is not correct";
    DrawSceneNode(node, this, recursively, program);
}
