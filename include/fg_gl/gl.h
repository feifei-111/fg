...
struct FG_API ShaderProgram {
    ...
    bool Init(const std::string& vertex_shader_path,
              const std::string& fragment_shader_path);
    bool Ready() const { return program_ != 0; }
    unsigned int ProgramID() const { return program_; }
    ...
    void UniformFloat3(const std::string& name, const glm::vec3& vec3) {
        glUniform3f(
          glGetUniformLocation(program_, name.c_str()), vec3.x, vec3.y, vec3.z);
    }
    void UniformFloatMat3Vec(const std::string& name,
                             float* data,
                             size_t mat_num = 1,
                             unsigned int transpose = GL_FALSE) {
        glUniformMatrix3fv(glGetUniformLocation(program_, name.c_str()),
                           mat_num,
                           transpose,
                           data);
    }
    void UniformFloatMat4Vec(const std::string& name,
                             float* data,
                             size_t mat_num = 1,
                             unsigned int transpose = GL_FALSE) {
...
struct FG_API Texture2D {
    ...
    unsigned int CurrentUnit() const { return tex_unit_; }
    void Delete() {
...
struct VBOTemplate {
    ...
    bool Ready() const { return vbo_ != 0; }
    // if reuse same vbo
    ...
    void SetAttr(int column_size, unsigned int norm = GL_FALSE) {
        glVertexAttribPointer(attr_idx_,
                              column_size,
                              GLDtype<Dtype>::value,
                              norm,
                              stride_,
...
struct FG_API EBO {
    ...
    bool Init(unsigned int* data,
              unsigned int data_size,
              unsigned int draw_type);
    bool Ready() const;
    void Bind() const { glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_); }
...
struct FG_API VAO {
    ...
    void Init() { glGenVertexArrays(1, &vao_); }
